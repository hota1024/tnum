import { Enum, EnumNarrowed, EnumNarrowSelector } from "@/core";
...
> extends Enum<OptVariants<T>, K> {
	...
	/**
	 * parse json to Opt<T> and returns it.
	 *
	 * @param json JSON string.
	 * @returns parsed Opt<T>.
	 */
	static fromJSON<T = unknown>(json: string): Opt<T> {
		const parsed = JSON.parse(json);

		return new Opt(parsed.variant, parsed.value);
	}
...
const name = optName.match({
	Some: (name) => name,
	_: () => "[no name]",
});

optName.unwrapOr("[no name]");
